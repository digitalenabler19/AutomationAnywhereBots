...
# Date: 13-08-2021
# V 1.0
# Migrate users from A360 control room to another A360 control room
...
def migrate_users(args):
    try:
        ...
        if auth_response_v11["status_code"] == 200:
            ...
            v11_users = get_users(v11_auth_token, SOURCE_CR_SERVER_URL)
        ...
        if auth_response_a360["status_code"] == 200:
            ...
            a360_users = get_users(a360_auth_token, TARGET_CR_SERVER_URL)

        if len(v11_users) > 0 and len(a360_users) > 0:
            for user in v11_users:
                if not check_user_in_a360(user["username"], a360_users):
                    create_user(a360_auth_token, TARGET_CR_SERVER_URL, user)
            ...
            for user in created_users:
                delete_user(a360_auth_token, TARGET_CR_SERVER_URL, user)
            """
            # ########################################################################
        else:
