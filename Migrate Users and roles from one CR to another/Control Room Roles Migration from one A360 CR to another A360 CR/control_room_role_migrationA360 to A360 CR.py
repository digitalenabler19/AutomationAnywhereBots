...
# Date: 13-08-2021
# V 1.0
# Migrate roles from A360 control room to another A360 control room
...
def migrate_roles(args):
    try:
        SOURCE_CR_SERVER_URL = args[0]
        SOURCE_CR_USERNAME = args[1]
        SOURCE_CR_USER_PASSWORD = args[2]
        TARGET_CR_SERVER_URL = args[3]
        TARGET_CR_USERNAME = args[4]
        TARGET_CR_USER_PASSWORD = args[5]

        initialize_logger(args[6], args[7])
        auth_response_a360 = authenticate(TARGET_CR_SERVER_URL, TARGET_CR_USERNAME, TARGET_CR_USER_PASSWORD)
        auth_response_v11 = authenticate(SOURCE_CR_SERVER_URL, SOURCE_CR_USERNAME, SOURCE_CR_USER_PASSWORD)
        ...
        if auth_response_v11["status_code"] == 200:
            ...
            v11_roles = get_roles(v11_auth_token, SOURCE_CR_SERVER_URL)
        ...
        if auth_response_a360["status_code"] == 200:
            ...
            a360_roles = get_roles(a360_auth_token, TARGET_CR_SERVER_URL)

        if len(v11_roles) > 0 and len(a360_roles) > 0:
            for role in v11_roles:
                if not check_role_in_a360(role["name"], a360_roles):
                    create_role(a360_auth_token, TARGET_CR_SERVER_URL, role)
            ...
            for role in created_roles:
                delete_role(a360_auth_token, TARGET_CR_SERVER_URL, role)
            """
            # ########################################################################
        else:
